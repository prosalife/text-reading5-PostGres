...
const all_roots = "all_roots.json"; // 
...
client.query('SELECT route_id FROM ' + view_name + ' GROUP BY route_id ORDER BY route_id').then(res => {
    ...
    let all_record = new Promise(async function(resolve, reject) {
        ...
        for (let i = 0; i < id_roots.length; i++) {
            ...
            one_root(id_roots[i].route_id); // вызываем функцию расчета одного маршрута one_root
        }
...
function one_root(id_route) {
    ...
    client.query('SELECT * FROM ' + view_name + ' WHERE route_id = ' + id_route).then(res => {
        ...
        for (let j = 0; j < data.length; j++) {
            ...
            obj_geo_next.root_name = data[j].route_name;
            obj_geo_next.platform = data[j].station_name;

            obj_geo_next.platform_number = data[j].seq_no;
            obj_geo_next.longitude = data[j].longitude;
            obj_geo_next.latitude = data[j].latitude;
